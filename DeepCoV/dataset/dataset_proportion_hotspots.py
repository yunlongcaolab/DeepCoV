...
from dataset.evo_dms_reader import read_dms
from dataset.evo_data_reader_hotspots import CountReader



class SARS2SpikeProteinDataset_EvoData(Dataset):
    """Dataset for mutational hotspot prediction.
    
    Specialized dataset for identifying and predicting mutation hotspots
    in SARS-CoV-2 evolution using DMS and sequence data.
    """
    def __init__(self,args, config,ds = None):
        ...
        ### modify
        if config['dataset']['path']['synthetic_count']  is not None:
            self.syn_msa = np.load(config['dataset']['path']['synthetic_count'])
    ...
    def __getitem__(self,index):
        ...
        cluster_index = item[f'{self.tag}_index']
        t0 = item['t0']
        location = item['location']
        cluster_name = item[f'{self.tag}_name']
        ### modify
        cluster_name_mut = item[f'{self.tag}_name_mut']
        ds = item['ds']  
        ...
        if ds == 'synthetic':
            params = dict(loc=location, # location
                        t0=t0, # t0日起
                        target_name=cluster_name, # target序列的名字
                        target_index=cluster_index,
                        target_msa = self.syn_msa['refmut_msa'][self.syn_msa['refmut_name'] == cluster_name_mut,:],
            ...
            out_a = self.reader_a.query_sample_by_name_syn(**params)
        else:
            params = dict(loc=location, # location
                        t0=t0, # t0日起
                        target_name=cluster_name, # target序列的名字
                        ...
                        n_bg_days=self.config['feature']['n_bg_days'], #背景的时间窗 180天
                        stride=self.config['feature']['stride'], # 背景的步长 3
                        shuffle=True) # 把一个样本点对应的series数据
        ...
        out['background_ratios'] = torch.tensor(out_a["ratio"]).to(torch.float32)
        out['t0_index'] = torch.tensor(out_a["t0_index"])
        out['location_index'] = torch.tensor(out_a["location_index"])
        out['target_name'] = torch.tensor(int(cluster_name[1:]))
