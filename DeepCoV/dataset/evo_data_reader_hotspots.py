...
class CountReader(object):
    """Reader for hotspot-specific evolution count data.
    
    Specialized CountReader for mutational hotspot analysis
    with enhanced filtering and processing capabilities.
    """
    ...
    ### modify
    def query_sample_by_name_syn(self, loc, t0, target_name, target_index, target_msa, top_k, n_bg_days=180, stride=3, shuffle=True):
        """
        ...
        out = dict(count=np.concatenate([np.zeros((1,background_values['count'].shape[1])), # target_values["count"][None],
                                         background_values["count"]], axis=0),
                   total=background_values["total"],
                   ratio=np.concatenate([np.zeros((1,background_values['ratio'].shape[1])),# target_values["ratio"][None],
                                         background_values["ratio"]], axis=0),
                   msa=np.concatenate([target_msa,# target_values["sequence"][None],
                                       background_values["msa"]], axis=0),
                   target_sequence_index=target_index,# target_values["index"],
    ...
    def get_t1_values(self, loc_index, seq_index, t0_index, min_total_isolates_t1=100.0):
        ...
        ### modify:
        if seq_index in self._sequence_names_mapper.values():
            target_isolates_t1 = self._count[loc_index, seq_index, t0_index+1: t0_index+61]
        else:
            print(f'{seq_index} is a new sequence')
            # target_isolates_t1 = np.zeros([60])
            target_isolates_t1 = np.zeros([total_isolates_t1.shape[0]])
        print(f'target_isolates_t1 shape: {target_isolates_t1.shape}')

        _total = np.where(total_isolates_t1 == 0.0, 1.0, total_isolates_t1)
        print(f'_total shape: {_total.shape}')
        print(f'total_isolates_t1 shape: {total_isolates_t1.shape}')
