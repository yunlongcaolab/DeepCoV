...
from model.model_proportion_continue import proportion_continue_t1 as Evo_prediction_proportion
from dataset.dataset_proportion_continue import SARS2SpikeProteinDataset_EvoData as SARS2SpikeProteinDataset
...
# set_seed(42)
# set_seed(520) # repeat2
# set_seed(721) # repeat3
set_seed(777)
...
def log_metrics_eval(config,
    ...
    mask = (target_ratio_t1_output.notna() & target_ratio_t1_label.notna())

 
    filtered_target_ratio_t1_output = target_ratio_t1_output[mask]
    filtered_target_ratio_t1_label = target_ratio_t1_label[mask]
    mse = mean_squared_error(filtered_target_ratio_t1_label,filtered_target_ratio_t1_output)
    rmse = mse**0.5
    ...
    wandb.log({
               f"{status_log}_mse": mse, 
               f"{status_log}_rmse": rmse,
    ...
    df_metrics = pd.DataFrame(metrics_history_eval)
    csv_file_path = os.path.join(config['other']['output_dir'], name,f"{status_log}_metrics-step-{global_step}.csv")
...
def _add_t1_infor(group ,val_dataset,config):
        ...
        group['target_isolates_t1'] = t1_info['target_isolates_t1'][group_names[3]-1]
        group['total_isolates_t1'] = t1_info['total_isolates_t1'][group_names[3]-1]
        group['target_ratio_t1_label'] = t1_info['target_ratio_t1'][group_names[3]-1]
        group['target_ratio_t1_mask'] = t1_info['target_ratio_t1_mask'][group_names[3]-1]
        return group
...
def save_to_csv_regres(config,dataset_csv,predict_time,date,location,target,outputs, file_path,val_dataset):
    ...
    ### for continue predict
    # results_pd_grouped = results_pd.groupby(["t0",'location',f'{tag}_name'])
    results_pd_grouped = results_pd.groupby(["t0",'location',f'{tag}_name','t1'])
    results_pd_added_t1 = results_pd_grouped.apply(_add_t1_infor,val_dataset=val_dataset,config=config)

    results_pd_added_t1.to_csv(file_path, index=False)

    return results_pd_added_t1['target_ratio_t1_output'],results_pd_added_t1['target_ratio_t1_label']
...
def validation(config,model,rank,local_rank,world_size,val_loader,val_dataset,name,criterion_regression,global_step,dataset_csv,status):
    ...
    with torch.no_grad():
        for val_step,val_batch in val_progress_bar:
            ...
            all_target_cluster_eval=torch.repeat_interleave(val_batch["target_name"].unsqueeze(-1),60,dim=1).flatten().tolist()
            all_date_eval=torch.repeat_interleave(val_batch["t0_index"].unsqueeze(-1),60,dim=1).flatten().tolist()
            all_location_eval=torch.repeat_interleave(val_batch["location_index"].unsqueeze(-1),60,dim=1).flatten().tolist()
            outputs_list=logits_regression.flatten().tolist()
            all_label_cla_eval=list(range(1,61)) * logits_regression.size(0)
        ...
        if rank == 0:
            ...
            concatenated_all_target_cluster_eval_gather_list_cat_out = [tag[0]+ str(i) for i in concatenated_all_target_cluster_eval_gather_list_cat.tolist()]
...
def train(config,start_epoch,model,rank,local_rank,world_size,train_loader,optimizer_model,epoch,global_step,criterion_regression,scheduler,sampler = None):
    ...
    for step, batch in progress_bar:
        ...
        #                                         vae_out = vae_out)
        loss = criterion_regression.loss_comput(logits_regression, 
                                                batch["target_ratio_t1"],
                                                t1_total_isolate_mask=batch["target_ratio_t1_mask"],
                                                t1_label_weight = batch["t1_label_weight"],
                                                vae_out = vae_out,
                                                t1_continue_day_weight =batch['t1_continue_day_weight'])
        loss.backward()
...
class combine_loss(object): 
    def __init__(self,config):
        ...
        self.config = config
    ### for continue prediction
    def loss_comput(self,y_pred,y_true,lambda_vae=1.0, lambda_reg=1.0,t1_total_isolate_mask = None,t1_label_weight=None,vae_out = None,t1_continue_day_weight =None):
        ...
        part1_mse_regression = torch.sum(p1 * normalized_element_weights * t1_continue_day_weight) 
...
def main(args):
    ...
    #===============================evaluate=====================================
        ...
        if config['dataset']['path']['use_to_evaluate_checkpoint_path'] is not None:
            ...
            Sampler_val = DistributedSampler(val_dataset,rank=local_rank,num_replicas=world_size,drop_last=True)
            val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=config['hyperparameters']['batch_size'],drop_last=True,sampler=Sampler_val,num_workers=4)
