...
# load embedding
RBD_emb=np.load(f'{test_data_embedding_dir}/RBD_sequences_test.npy')#.astype(np.float16)
HSEQ_emb=np.load(f'{ori_data_embedding_dir}/antibody_heavy_sequence_all_data.npy')#.astype(np.float16)
LSEQ_emb=np.load(f'{ori_data_embedding_dir}/antibody_light_sequence_all_data.npy')#.astype(np.float16)

# scale
mean_rbd=np.load(f'{test_data_embedding_dir}/RBD_embedding_flatten_mean_test_JN1era_add.npy')
std_rbd=np.load(f'{test_data_embedding_dir}/RBD_embedding_flatten_std_test_JN1era_add.npy')
mean_hseq=np.load(f'{ori_data_embedding_dir}/HSEQ_embedding_flatten_mean.npy')
std_hseq=np.load(f'{ori_data_embedding_dir}/HSEQ_embedding_flatten_std.npy')
mean_lseq=np.load(f'{ori_data_embedding_dir}/LSEQ_embedding_flatten_mean.npy')
std_lseq=np.load(f'{ori_data_embedding_dir}/LSEQ_embedding_flatten_std.npy')
...
df_RBD = pd.read_csv(f'{test_data_dir}/RBD_seq_escape_test_JN1era.csv').values

emb_dict_RBD = dict()
for i in range(len(df_RBD)):
    # emb_dict_RBD[str(df_RBD[i][0])+df_RBD[i][1]] = RBD_emb[i]
    emb_dict_RBD[str(df_RBD[i][0])] = RBD_emb[i]

df_hseq = pd.read_csv(f'{ori_data_dir}/antibody_heavy_sequence.csv').values
emb_dict_hseq = dict()
for i in range(len(df_hseq)):
    emb_dict_hseq[df_hseq[i][0]] = HSEQ_emb[i]
    
df_lseq = pd.read_csv(f'{ori_data_dir}/antibody_light_sequence.csv').values
emb_dict_lseq = dict()
...
if __name__ == '__main__':
    ...
    L_SEQ_LEN=105
    model_i =9
    model_dir=f"/lustre/grp/cyllab/yangsj/evo_pred/1article/benchmark/E2VD/model/escape/result_200_a0.88_lr0.0001_regk1.2_k5_f1_prop_loss_/model_{model_i}.pth.tar"
    
    test_df = pd.read_csv(f'{test_data_dir}/RBD_seq_escape_test_JN1era.csv')
    ...
    df = pd.DataFrame({"rbd_index": val_reg_label, "regression_output": y_reg, "classification_output": y_cls})
    df.to_csv(f'{res_dir}/E2VD_escape_output_test_JN1era.csv',index=False)
