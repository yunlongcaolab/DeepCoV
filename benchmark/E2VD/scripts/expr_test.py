...
class ContextPooling(nn.Module):
    def __init__(self,seq_len,in_dim=1280):
...
if __name__ == '__main__':
    ...
    BATCH_SIZE = 32
    SEQ_LEN=201
    model_dir="/lustre/grp/cyllab/yangsj/evo_pred/1article/benchmark/E2VD/model/expr/result_100_a0.95_lr2e-05_regk1_k5_f1_prop_/model_{}.pth.tar"
    ...
    if 1:
        # load train data
        train_feats=np.load(f'{ori_data_embedding_dir}/single_expr_esm2_train_data.npy')
        ...
        # load test data
        test_feats=np.load(f'{test_data_embedding_dir}/single_expr_esm2_test_data.npy')
        test_reg_label=np.load(f'{test_data_embedding_dir}/single_expr_esm2_test_label.npy')
    ...
    with torch.no_grad():
        for model_i in range(model_n):
            ...
            df = pd.DataFrame({"rbd_index": test_reg_label, "regression_output": y_reg, "classification_output": y_cls})
            df.to_csv(f'{res_dir}/E2VD_expression_output_test_JN1era_fold{model_i}.csv',index=False)
