...
out_dir='/lustre/grp/cyllab/share/evolution_prediction_dl/benchmark/MLR/analysis'
...
for(n_pred_top in c(n_truth_top:n_pred_top_max)){
    ...
    mlr_set =  mlr %>% filter(t0 == unique_dates[i],rbd_name_mut %in% rbd_set) %>% arrange(-median_freq_forecast) %>% slice(1:n_pred_top) %>% pull(rbd_name_mut)
    ...
    if(n_truth_top ==1){
        evaluation = data.frame(date=window_start,n_pred_top=n_pred_top,n_truth_top=n_truth_top,
                                EVEscape=(truth_set %in% EVEscape_set),E2VD=(truth_set %in% E2VD_set),ga = (truth_set %in% ga_set),
                                DeepCoV=(truth_set %in% ours_set),mlr=(truth_set %in% mlr_set))
    }
    else{
        evaluation = data.frame(date=window_start,n_pred_top=n_pred_top,n_truth_top=n_truth_top,
                                EVEscape=jaccard_index(truth_set,EVEscape_set),E2VD=jaccard_index(truth_set,E2VD_set),ga = jaccard_index(truth_set,ga_set),
                                DeepCoV=jaccard_index(truth_set,ours_set),mlr=jaccard_index(truth_set,mlr_set))
    }
    return(evaluation)
    })
