...
summary_success = rank_eval_results %>% group_by(n_pred_top,n_truth_top) %>% 
    ...
    summarize(EVEscape = mean(EVEscape),E2VD=mean(E2VD),DeepCoV=mean(DeepCoV),ga=mean(ga),mlr=mean(mlr)) %>% 
...
pdf(str_glue('{plot_save_dir}/success_rate_predtopK_truthtop1.pdf'),width=3.7,height=3)
p
dev.off()
...
p = data_plot %>% 
  ...
  theme_fontsize + 
  theme(legend.position = 'bottom')
pdf(str_glue('{plot_save_dir}/jaccard_index_predtopK_truthtop{n_truth_top}.pdf'),width=3.7,height=3)
p
dev.off()
