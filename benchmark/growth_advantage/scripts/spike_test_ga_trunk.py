...
# 读取数据
spike_dataset = pd.read_csv(args.dataset_path)
in_file = "/lustre/grp/cyllab/yangsj/evo_pred/0article/data/processed/241030/spike/spike_count_smooth.npz"
...
def GA_from_past(DF_calc):
    def calc_ga_cov(row):
        seq = func.seq_count_by_date(row['location'], row['spike_name'], '2019-12-24', tb=row['t0'])
...
GA_calc.to_csv(f'{args.save_dir}/results/spike_test_GA_trunk{str(args.subset)}.csv',index=False)
