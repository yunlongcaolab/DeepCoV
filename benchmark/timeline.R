...
mlr = read_csv(str_glue('{data_dir}/benchmark/MLR/results/TestFull_2023-10-01_to241030_MLR_pred30days.csv'))%>% 
      mutate(rbd_name_mut= rbd_name_mapper[rbd_name] %>% unname %>% as.character) %>% mutate(median_freq_forecast = ifelse(is.na(median_freq_forecast),0,median_freq_forecast))
...
ga_rank_smooth <- ga %>% arrange(rbd_name_mut, t0) %>% group_by(rbd_name_mut) %>%
  mutate(ga_smooth_7d = rollapply(ga,width = 7,FUN = min,align = "center",fill = NA,na.rm = TRUE)) %>% 
  ungroup() %>%group_by(t0) %>%
  mutate(ga_rank = rank(-ga_smooth_7d))
  

### rank of dominant strains
bind_rank = bind_rows(
    ours %>% group_by(t0) %>% mutate(ours_rank = rank(-target_ratio_t1_output)) %>% form_rank_df(.,method = 'ours', rank_col = 'ours_rank', date_col='t0', major_strain=c('JN.1','KP.2','KP.3'),topn =topn1, Event=str_glue('rank top{topn1}')),
    mlr %>% group_by(t0) %>% mutate(mlr_rank = rank(-median_freq_forecast))  %>% form_rank_df(.,method = 'mlr', rank_col = 'mlr_rank', date_col='t0', major_strain=c('JN.1','KP.2','KP.3'),topn =topn1, Event=str_glue('rank top{topn1}')),
    ga_rank_smooth %>% form_rank_df(.,method = 'ga', rank_col = 'ga_rank', date_col='t0', major_strain=c('JN.1','KP.2','KP.3'),topn =topn1, Event=str_glue('rank top{topn1}')),
    ...
    ours %>% group_by(t0) %>% mutate(ours_rank = rank(-target_ratio_t1_output)) %>% form_rank_df(.,method = 'ours', rank_col = 'ours_rank', date_col='t0', major_strain=c('JN.1','KP.2','KP.3'),topn =topn2, Event=str_glue('rank top{topn2}')),
    mlr %>% group_by(t0) %>% mutate(mlr_rank = rank(-median_freq_forecast))  %>% form_rank_df(.,method = 'mlr', rank_col = 'mlr_rank', date_col='t0', major_strain=c('JN.1','KP.2','KP.3'),topn =topn2, Event=str_glue('rank top{topn2}')),
    ga_rank_smooth %>%form_rank_df(.,method = 'ga', rank_col = 'ga_rank', date_col='t0', major_strain=c('JN.1','KP.2','KP.3'),topn =topn2, Event=str_glue('rank top{topn2}')),
...
timeline_data = bind_rows(bind_rank %>% 
    left_join(ours %>% filter(target_ratio_t0 >= 0.2) %>% group_by(rbd_name_mut) %>% summarise(date_end = min(t0), .groups = "drop"), by = "rbd_name_mut") %>% mutate(date_top = if_else(date_top > date_end, as.Date(NA), date_top)) %>% select(-date_end),
...
timeline_data = timeline_data %>% 
    mutate(direction = ifelse(rbd_name_mut %in% c('JN.1','KP.3'),1,-1),
          position = case_when(
                               Event %>% str_detect(coll('rank(2m) top10'))==TRUE ~ 1,Event %>% str_detect(coll('rank top10'))==TRUE ~ 1,
...
timeline_data %>% write_csv(str_glue('{analysis_data_dir}/timeline_data.csv'))
...
timeline_data_draw = timeline_data %>% mutate(position = position*direction,text_position = position+0.5*direction)%>%
    ...
    mutate(method = factor(method,levels = c("truth","DeepCoV","EVEscape","E2VD","mlr","ga")))  %>% # change
...
pdf(str_glue('{plot_save_dir}/timeline.pdf'), height=12, width=5)
p
dev.off()