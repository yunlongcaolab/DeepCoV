...
draw_mut2site_score = function(data_score,title,site_show_normal = c(346,456,478,475),tag='rbd'){
    if(tag == 'rbd'){
        ...
        breaks_k = seq(1,301,5)
        limits_k = c(1,301)
        sites = c(1:301)
   }
    dat = data_score %>% group_by(site,site_name) %>% summarize(score = mean(score)) %>%  
        right_join(data.frame(site = sites)) %>% 
        mutate(score = ifelse(is.na(score),0,score)) %>% ungroup

   site_high = dat %>% filter(score > 0) %>% arrange(-score) %>% dplyr::slice(1:1) %>% pull(site)
   print(site_high)
   ...
   geom_point(color="#A03429", shape=21)+ theme_classic() + theme(
  ...
  )+ scale_x_continuous(breaks=breaks_k,limits=limits_k,labels = function(x) site_mapper[x]) + 
    ...
    geom_label_repel(aes(label=ifelse(site %in% site_show,site_name, '')), min.segment.length = 0, direction="both", fill = alpha(c("white"),0.5), max.overlaps=50) +
    ggtitle(title)
...
save_dir = '/lustre/grp/cyllab/yangsj/evo_pred/0article/insilico_mutational_hotspot_scanning/results/spike_single_JN1era_insinclude'
# res = read_csv('/lustre/grp/cyllab/yangsj/evo_pred/0article/insilico_mutational_hotspot_scanning/results/spike_single_JN1era/KP3-2024-06-01to2024-09-01_regres_outputs_labels-step-18485.csv') %>%
#     filter(spike_name_mut %>% str_detect('ins') == FALSE) # JN1/BA2 insertion site are not included
res = read_csv('/lustre/grp/cyllab/yangsj/evo_pred/0article/insilico_mutational_hotspot_scanning/results/spike_single_JN1era/KP3-2024-06-01to2024-09-01_regres_outputs_labels-step-18485.csv')
postfix = ''
base_strain='KP.3'
location='Global'
site_show_normal = c(31,16,17,18,19,20)
min_thres = 0.00
...
res_mut =res %>% filter(!!sym(str_glue('{tag}_name_mut')) %>% str_detect(coll('+')) == T)  %>% 
           ...
           mut = !!sym(str_glue('{tag}_name_mut')) %>% str_split(coll('+')) %>% map_chr(~.[[2]]),site_name = mut %>% str_sub(2,-2),# site = mut %>% str_extract('\\d+ins\\d+|\\d+') %>% as.numeric,
           aa_from = mut %>% str_extract('^[A-Z]'),aa_to = mut %>% str_extract('[A-Z-]$') )
site_mapper = unique(res_mut$site_name)
names(site_mapper) = 1:length(unique(res_mut$site_name))
DFsite_mapper = tibble(site=1:length(unique(res_mut$site_name)),site_name = unique(res_mut$site_name))
res_mut = res_mut %>% left_join(DFsite_mapper)

date=res_mut %>% pull(date) %>% unique
dat = res_mut %>% filter(location == location,base_strain == base_strain) %>% 
    select(!!sym(str_glue('{tag}_name_mut')) ,base_strain,date,mut,site,site_name,aa_from,aa_to,target_ratio_t1_output) %>% arrange(-target_ratio_t1_output) 
...
pdf(str_glue('{save_dir}/hotspots_{base_strain}_{location}_period_{min(dat_hm_ori$date)}to{max(dat_hm_ori$date)}_min{min_thres}{postfix}.pdf'),width = 9,height = 2.5) # ,width = 6
ps_ori
dev.off()
...
#             scale_x_continuous(breaks = 1:ncol(dat_logo_ori),labels = function(x) site_mapper[colnames(dat_logo_ori)] %>% unname) + # labels = colnames(dat_logo_ori), labels = function(x) site_mapper[colnames(dat_logo_ori)[x]],labels = function(x) site_mapper[x]) + 
#           theme_bw() + 
